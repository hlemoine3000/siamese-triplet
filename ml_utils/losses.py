...
class DualtripletLoss(nn.Module):
    ...
    def forward(self, sa, sp, sn, ta, tp, tn):
        ...
        tgt_distance_positive = (ta - tp).pow(2).sum(1)  # .pow(.5)
        tgt_distance_negative = (ta - tn).pow(2).sum(1)  # .pow(.5)

        loss1 = self.lamda[0] * F.relu(src_distance_positive - src_distance_negative + self.margin).mean()
        loss2 = self.lamda[1] * F.relu(tgt_distance_positive - tgt_distance_negative + self.margin).mean()
